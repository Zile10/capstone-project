...
module.exports = {
  getProducts(req, res) {
    req.user
    con.query("SELECT prodID, prodName, prodDesc, price, stock, imgURL, category FROM products", (err, result) => {
      if (err) throw err;
      res.status(200);
      res.send(result);
    });
  ...
  getProduct(req, res) {
    con.query("SELECT prodID, prodName, prodDesc, price, stock, imgURL, category FROM products WHERE productID = ?", [req.params.id], (err, result) => {
      if (err) throw err;
      res.send(result[0]);
    });
  },
